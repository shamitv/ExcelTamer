...
Each opened workbook is owned by an internal `ExcelAutomation` instance stored
in the process-wide session. Engine functions retrieve that instance and call
xlwings on the server execution thread.
...
3. Close workbooks through `excel.close` so handles are removed from the
   session.
